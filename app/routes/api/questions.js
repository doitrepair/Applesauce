		...
		.post(function(req, res) { 		//expected request syntax '"Question_text", "Question_summary"'
			db_connection.getConnection(function(err, tempConnection) {
                if (err) {
                	...
                	tempConnection.query('INSERT INTO questions (question_text, q_sum) VALUES (?, ?)', [req.body.text, req.body.sum], function (err, rows, fields) {
                		tempCont.release();
                    	if (err) {
		...
		.get(function(req, res) {
            db_connection.getConnection(function (err, tempConnection) {
                if (err) {
                    ...
                    console.log('Connection established!');
                    tempConnection.query('SELECT * FROM questions', function (err, rows, fields) {
                        tempCont.release();
                        if (err) {
                            console.log("Error with query");
                        } else {
		...
		.get(function(req, res) { //expects single integer id
            db_connection.getConnection(function(err, tempConnection) {
                if (err) {
                    ...
                    console.log('Connection established!');
                    tempConnection.query('SELECT * FROM questions WHERE question_id = ?', req.params.id, function(err, result){
                        tempCont.release();
                        if (err) {
                            console.log("Error with query");
                        } else {
		...
		.put(function(req, res) { //expected syntax ["Question text", "Question Summary", ID_Number]
            db_connection.getConnection(function(err, tempConnection) {
                if (err) {
                    ...
                    console.log('Connection established!');
                    tempConnection.query('UPDATE questions SET question_text = ?, q_sum = ? WHERE question_id = ?', [req.body.text, req.body.sum, req.params.id], function(err, result){
                        tempCont.release();
                        if (err) {
                            console.log("Error with query");
                        } else {
		...
		.delete(function(req, res) {
            ...
            db_connection.getConnection(function(err, tempConnection) {
                if (err) {
                    ...
                    console.log('Connection established!');
                    tempConnection.query('DELETE FROM questions WHERE question_id = ?', req.params.id, function(err, result){
                        tempCont.release();
                        if (err) {
                            console.log("Error with query");
                        } else {
